// 분봉 로컬 합성 — WS 체결가 틱을 분 버킷(기본 5분, TREND_BAR_MINUTES)으로 잘라 종가 링을 유지한다(추세 4선의 입력).
// 2026-08-19: 첫날 42건 분석(docs/분석/2026-08-19_추세-첫날-42건-분석.md) — 1분봉은 거래 수×슬리피지(실측 0.65%/진입)에
// 구조적으로 진다(재현 −98%). 3분봉은 거래 1/3·건당 이익 4배라 봉 주기를 3분으로 올렸다.
// 2026-08-21: 상태기계 전환과 함께 3분 → 5분(docs/분석/2026-08-21_4선-상태기계-검증.md §6). 3일 재현 합계
// 1분 −$76 / 3분 +$75 / 5분 +$95.53, 5분만 3일 전부 비음수 — 봉이 길수록 가짜 플립이 줄어 확인봉 없이도 신호가 깨끗하다.
// 봉 키는 여전히 "epoch 분"이고 봉 시작 분(barMinutes의 배수)으로 정규화한다 — 토스 min:5 봉의 dt(봉 시작)와 같은
// 버킷(ET −240/−300분·KST +540분 오프셋이 모두 5의 배수라 정렬 일치. 3분일 때와 같은 근거).
...
/** 추세 봉 주기(분) — 신호·시드·봉 빌더가 같은 값을 읽는다. 3으로 두면 옛 동작으로 한 줄 롤백. */
export const TREND_BAR_MINUTES = 5;
