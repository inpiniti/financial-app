...
- **1행 = 1에피소드(세트)**: 급등(진입) 신호가 행을 열고(insert), 같은 종목의 **이탈(트레일링 하락 확정)** 신호가 행을 닫는다(update). 단독 하락은 감지·기록하지 않는다(2026-08-13 이탈 재정의 — 세트만 의미 있음). `plunge_*` 컬럼명은 스키마 초안의 흔적으로, **이탈(하락 확정) 시점 값**을 담는다.
...
| status | 의미 | 전이 |
|---|---|---|
| `open` | 급등 기록됨, 이탈 대기 중 | insert 시 기본값 |
| `closed` | 이탈(트레일링 하락) 확정으로 정상 종결 — 변동율 3종 채워짐 | 이탈 update |
| `expired` | 타임아웃(기본 30분)까지 급락 없음 — 급락 컬럼 null 유지 | 타임아웃 update |
| `plunge_only` | ~~단독 급락~~ — **미사용**(이탈 재정의로 폐기). enum·정책에는 남아 있으나 앱이 만들지 않는다 | — |
...
- **단독 하락은 기록하지 않는다**(이탈 재정의): 이탈(EXIT)은 감지기의 급등 후 추적 모드에서만 발화하므로 항상 열린 에피소드와 세트다. 열린 에피소드 없는 이탈 신호(stop/재시작 직후 등)는 버린다. 에피소드는 급등 시점부터 호가를 잡고 있으므로 이탈 시점 매수1/2호가는 대부분 채워진다.
