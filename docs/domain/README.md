...
## 1-1. 운영 원칙 (2026-09-11)

- 자동매매 운영 전략은 **실시간 MA5 단일 전략**을 우선한다.
- 모델·추세·기울기·사다리 경로는 실거래 기본 경로가 아니라 롤백/실험 보존 경로로 다룬다.
- 책임 경계(신호/포지션/실행/설정)와 실패 위치 식별 기준은 아래 문서를 정본으로 삼는다.
	- `docs/domain/오토파일럿/2026-09-11_실시간5선-단일전략-도메인-정의.md`
...
| 5선 돌파 (문서 없음 — ADR 0006·0007·0010·0012) | **현행 기본 (설정 `engineMode='martingale'`, 기본값)** | `core/martingale`, `features/scalper/martingaleMode.ts` | 1분봉 5선 상향 돌파 봉 ∧ 엔진 옵션 진입 필터(기본 5선 상승)에 매수(프리·정규·애프터만) — 진행 중 봉 실시간 판정(`MARTINGALE_LIVE_ENTRY`, 봉당 1회). 옵션 물타기(기본 켬): 보유 중 평단 −k%(k≥3)에서 보유량 ×(k−1). 익절 +3% · 손절 없음 · 19:55 ET 마감 청산 |
| 엔진 옵션 (문서 없음 — ADR 0012) | **현행 (설정 `engineOptions`)** | `core/martingale`(EntryFilters·AveragingDownRule), `features/scalper/engineMode.ts` | 세 엔진 공통 중복 선택: 진입 필터(정배열·5선 상승·4선 모두 상승, AND) + (k−1)배 물타기. 모델·기울기 엔진에는 BUY 게이트·데코레이터로 얹힘 |
