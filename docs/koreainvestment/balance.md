> 원문: balance.txt (KIS API 포탈 복사본) — 변환 시 필드 추가/삭제 없음
...
| pchs_rmnd_wcrc_amt | 매입잔액원화금액 | String | Y | 19 | (원문에 없음) |
...
| unit_amt | 단위금액 | String | Y | 19 | (원문에 없음) |
| std_pdno | 표준상품번호 | String | Y | 12 | (원문에 없음) |
| prdt_type_cd | 상품유형코드 | String | Y | 3 | (원문에 없음) |
| scts_dvsn_name | 유가증권구분명 | String | Y | 60 | (원문에 없음) |
...
| Element | 한글명 | Type | Required | Length | Description |
|---|---|---|---|---|---|
| output2 | 응답상세2 | Array | Y | - | (원문에 없음) |

#### output2 (배열 원소)

| Element | 한글명 | Type | Required | Length | Description |
|---|---|---|---|---|---|
| crcy_cd | 통화코드 | String | Y | 3 | (원문에 없음) |
| crcy_cd_name | 통화코드명 | String | Y | 60 | (원문에 없음) |
...
| frst_bltn_exrt | 최초고시환율 | String | Y | 31 | (원문에 없음) |
| frcr_buy_mgn_amt | 외화매수증거금액 | String | Y | 31 | 매수증거금으로 사용된 외화금액 |
| frcr_etc_mgna | 외화기타증거금 | String | Y | 31 | (원문에 없음) |
| frcr_drwg_psbl_amt_1 | 외화출금가능금액1 | String | Y | 29 | 출금가능한 외화금액 |
| frcr_evlu_amt2 | 출금가능원화금액 | String | Y | 29 | 출금가능한 원화금액 |
| acpl_cstd_crcy_yn | 현지보관통화여부 | String | Y | 1 | (원문에 없음) |
| nxdy_frcr_drwg_psbl_amt | 익일외화출금가능금액 | String | Y | 31 | (원문에 없음) |

Body(계속)

| Element | 한글명 | Type | Required | Length | Description |
|---|---|---|---|---|---|
| output3 | 응답상세3 | Object | Y | - | (원문에 없음) |
...
| dncl_amt | 예수금액 | String | Y | 19 | (원문에 없음) |
| cma_evlu_amt | CMA평가금액 | String | Y | 19 | (원문에 없음) |
| tot_dncl_amt | 총예수금액 | String | Y | 19 | (원문에 없음) |
| etc_mgna | 기타증거금 | String | Y | 19 | (원문에 없음) |
| wdrw_psbl_tot_amt | 인출가능총금액 | String | Y | 19 | (원문에 없음) |
| frcr_evlu_tota | 외화평가총액 | String | Y | 19 | (원문에 없음) |
| evlu_erng_rt1 | 평가수익율1 | String | Y | 31 | (원문에 없음) |
| pchs_amt_smtl_amt | 매입금액합계금액 | String | Y | 19 | (원문에 없음) |
| evlu_amt_smtl_amt | 평가금액합계금액 | String | Y | 19 | (원문에 없음) |
| tot_evlu_pfls_amt | 총평가손익금액 | String | Y | 31 | (원문에 없음) |
| tot_asst_amt | 총자산금액 | String | Y | 19 | (원문에 없음) |
| buy_mgn_amt | 매수증거금액 | String | Y | 19 | (원문에 없음) |
| mgna_tota | 증거금총액 | String | Y | 19 | (원문에 없음) |
| frcr_use_psbl_amt | 외화사용가능금액 | String | Y | 20 | (원문에 없음) |
| ustl_sll_amt_smtl | 미결제매도금액합계 | String | Y | 19 | (원문에 없음) |
| ustl_buy_amt_smtl | 미결제매수금액합계 | String | Y | 19 | (원문에 없음) |
| tot_frcr_cblc_smtl | 총외화잔고합계 | String | Y | 29 | (원문에 없음) |
| tot_loan_amt | 총대출금액 | String | Y | 19 | (원문에 없음) |
