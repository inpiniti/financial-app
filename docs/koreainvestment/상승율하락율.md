> 원문: 상승율하락율.txt (KIS API 포탈 복사본) — 변환 시 필드 추가/삭제 없음

# 해외주식 상승율/하락율[해외주식-041]

[해외주식] 시세분석 / REST

> ⚠ 같은 GUBN(구분) 파라미터를 쓰는 가격급등락[해외주식-038]과 혼동 주의.
> 가격급등락은 **MINX(분 단위)** 시간창이지만, 이 API는 **NDAY(일 단위)** 다.
