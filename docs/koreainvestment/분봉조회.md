> 원문: 분봉조회.txt (KIS API 포탈 복사본) — 변환 시 필드 추가/삭제 없음

> ✅ **편집자 주 (2026-07-29)**: 원문이 **해외주식분봉조회(종목 분봉, HHDFS76950200)**로 교체되었다. 이전에 이 파일에 있던 해외지수분봉조회(FHKST03030200) 문서는 사용자가 정정하여 제거됨 — 종목 분봉은 이 문서가 스펙이다.
...
| 항목 | 값 | 항목 | 값 |
|---|---|---|---|
| Method | GET | URL | /uapi/overseas-price/v1/quotations/inquire-time-itemchartprice |
...
* 한국투자 Github에서 해외주식 분봉 다음조회 파이썬 샘플코드 참고하실 수 있습니다: https://github.com/koreainvestment/open-trading-api/blob/main/rest/get_ovsstk_chart_price.py
...
※ 지연시세 지연시간 : 미국 - 실시간무료(0분 지연, 나스닥 마켓센터에서 거래되는 호가 및 호가 잔량 정보) / 홍콩, 베트남, 중국, 일본 - 15분지연
...
> 원문 특이사항(보존): 헤더 appsecret 설명이 "appkey"로 오기. `output1`/`output2`의 Type 표기가 각각 "Object Array"/"Object(+array)"로 다소 모순 — 실제 응답은 output1이 메타 1건, output2가 분봉 배열(샘플코드 기준). 예시 dataclass의 `field` import 누락은 원문 그대로.
