> 원문: 미체결내역.txt (KIS API 포탈 복사본) — 변환 시 필드 추가/삭제 없음
...
- 포탈 복사 잔재로 요청/응답 Header·Query Parameter 표 캡션이 "주식주문(현금[V1_국내주식-001]) …"으로 잘못 찍혀 있음 — 실제 API와 무관한 템플릿 텍스트(제거함).
- Query Parameter `SORT_SQN` 설명란에 `[header tr_id: TTTS3018R]` / `""(공란)`이 원문 그대로 섞여 있음(다른 문서의 파라미터 분기 표기가 잘못 붙은 것으로 보이나 원문 그대로 보존).
- `ord_dt`~`splt_buy_attr_name` 등 output 필드명 다음 줄에 빈 줄이 하나씩 들어간 원문 서식(포탈 표 개행 잔재) — 변환 시 필드명·설명은 그대로 옮기고 개행만 정리.
- 모의투자 완전 미지원 — 대체 경로(주문체결내역 API의 `nccs_qty`)가 원문에 명시돼 있음.
