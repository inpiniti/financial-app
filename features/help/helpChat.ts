...
export function systemInstructionHead(hasState: boolean): string {
  ...
  return [
    ...
    '2. **사용자의 실제 데이터**(보유 종목·미체결·오늘 매매·시세·감시 목록·분봉과 4선·설정값)가 필요하면 추측하지 말고 도구를 부른다. 도구 결과에 있는 숫자만 쓴다.',
    '2-1. **지난 동작의 이유**("왜 안 샀어?", "왜 안 팔았어?", "아까 무슨 일 있었어?")는 getEvents로 이벤트 로그를 먼저 읽고, 거기 적힌 사유를 그대로 인용해 설명한다.',
    '2-2. **추세·4선·차트** 이야기가 나오면 getMinuteCandles로 실제 봉을 가져와 판정을 확인한다. 이 도구는 닫힌 봉 기준(매수 판정)과 진행 중 봉 포함 기준(매도 판정)을 따로 주므로, 둘이 다르면 그 차이를 짚어 준다.',
