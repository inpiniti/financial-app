...
- **도움말 챗봇(이 대화)** — 설명서만 읽는 게 아니라 필요하면 앱 데이터를 직접 조회해요: 자동 트레이딩 상태·감시 목록, 계좌 보유 종목·미체결, 오늘 매매 기록, 종목 시세·검색, 종목 기사 본문(Yahoo), 인터넷 뉴스 검색(Google 뉴스 — 제목·언론사·날짜까지). 전부 **조회만** 하고, 주문·시작·정지·설정 변경은 하지 못해요. 그런 건 화면에서 직접 해야 해요.
