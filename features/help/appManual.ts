...
const UP = pct(MODEL_CONFIG.takeProfitPct);
const DN = pct(MODEL_CONFIG.stopLossPct);
const HOLD_MIN = MODEL_CONFIG.timeoutMinutes;
...
거래가 활발한 종목을 순위에서 자동으로 모아 감시하다가, **예측 모델**이 오를 자리라고 본 종목을 사고 정해진 익절·손절·시간 선에 닿으면 팔아요.
...
## 4. 매매 규칙 (핵심)
**살 때는 예측 모델이, 팔 때는 정해진 가격선이** 정해요. 이 값들은 설계 고정값이라 설정에서 바꿀 수 없어요.

- **살 때 (모델)** — ${MODEL_BAR_MINUTES}분봉이 하나 닫힐 때마다, 리스트에 있는 종목 전부에 대해 모델이 **"지금 사면 ${DN} 떨어지기 전에 ${UP} 오를 확률"**을 계산해요. 그 확률이 기준값을 넘는 종목만 사요. 기준값은 3년 반치 과거 데이터에서 **상위 1%**에 해당하는 값이라 웬만해선 신호가 안 나요 — 하루에 몇 번 없는 게 정상이에요.
  - 모델이 보는 것: 최근 봉들의 등락률·봉 모양·거래량 변화·이동평균·RSI·MACD·스토캐스틱·볼린저·ATR·당일 시가 대비·전일 종가 대비 등 **33가지 지표**를 한꺼번에 봐요. 사람이 규칙을 정한 게 아니라, 과거에 실제로 ${UP}까지 갔던 자리들의 공통점을 기계가 찾아낸 거예요.
  - **정규장에서만** 사요. 프리마켓·애프터마켓·주간거래 시간에는 신호를 내지 않아요 — 모델이 정규장 데이터로만 배웠기 때문이에요.
  - 그날 거래대금이 200만 달러에 못 미치는 종목, 1달러 이하 종목은 걸러요.
- **팔 때 (세 가지 중 먼저 오는 것)** — 전부 산 가격(평단) 기준이에요.
  1. **${UP} 오르면** 전량 매도(익절)
  2. **${DN} 떨어지면** 전량 매도(손절)
  3. **${HOLD_MIN}분이 지나면** 어느 쪽에도 안 닿았어도 전량 매도(시간 청산)

  봉이 닫히기를 기다리지 않아요 — 체결가가 선에 닿는 순간 판단해요. 매도 주문은 체결될 때까지 현재가를 따라가고 도중에 거두지 않아요.
- **물타기** — **하지 않아요.** 한 종목은 한 번 사고 한 번 팔아요. 모델이 배운 정답이 "산 가격 기준 ${UP}/${DN}"이라, 평단을 낮추면 아예 다른 게임이 돼요.
- **직접 팔기** — "그리드 관리"의 게이지를 **두 번 연속 누르면** 확인 창이 뜨고, "매도하기"를 누르면 그 종목을 **전량** 매도해요. 세 청산선을 기다리지 않아요. 판다는 판단만 사람이 하고, 주문은 앱이 평소와 똑같이 내요 — 체결될 때까지 현재가로 따라가요. 기록에는 청산 사유가 "USER_SELL"로 남아요.
...
- **얼마나 믿을 수 있나** — 이 규칙은 2023년 3월~2026년 4월 데이터로 만들고, **한 번도 안 본 2026년 5~8월 구간**에서 3,116번의 매매를 흉내 내 검증했어요. 승률은 39%지만 이기면 ${UP}, 지면 ${DN}이라 비용을 빼고 거래당 평균 +0.42%였고 4개월 모두 플러스였어요. 다만 **12번 연속으로 진 구간도 있었어요** — 한 번 지면 ${DN}이라는 걸 감당할 수 있는 금액으로만 하세요. 그리고 흉내 낸 체결과 실제 체결은 다를 수 있어요.
...
4. **보유** — 산 종목은 "그리드 관리" 게이지에 나타나요. 게이지의 위끝이 익절선(+${UP}), 아래끝이 손절선(−${DN})이에요. 체결이 들어올 때마다 청산 조건을 봐요.
...
**청산 사유**는 일곱 가지예요 — 익절(+${UP} 도달) · 손절(−${DN} 도달) · 시간 청산(${HOLD_MIN}분 경과) · 사용자 정지 · 서킷 · 앱 밖 매도(한투 앱 등에서 직접 판 걸 잔고로 알아챈 경우) · 직접 팔기(게이지를 두 번 눌러 요청한 전량 매도).
...
청산선이 딱 세 개예요 — +${UP}(익절) · −${DN}(손절) · ${HOLD_MIN}분(시간). 먼저 닿는 것으로 팔아요. 기록의 청산 사유를 보면 어느 쪽이었는지 알 수 있어요.

**Q. 물타기는 안 하나요?**
하지 않아요. 모델이 배운 정답이 "산 가격 기준 +${UP}/−${DN}"이라, 평단을 낮추면 판정의 근거가 사라져요.
