...
- **홈 하단 메뉴 4칸**
  - **트레이딩** — 자동 매매의 본 화면. 상태 배지, 오늘 성과, 시작·정지 버튼, 그리드 관리 게이지, 트레이딩 리스트(종목마다 모델 판정 한 줄과 "매수 후보" 배지), 오늘 거래 기록, 기록(이벤트 로그). 모델 판정 줄은 확률만이 아니라 **모델이 쉬는 이유**도 말해요 — "정규장에만 사요", "거래대금 미달", "봉 부족"처럼요. 확률이 떠 있는데 안 사는 건 기준값(과거 상위 1%)에 못 미쳐서예요. 장이 닫혀 새 봉이 없을 때는 마지막으로 받은 봉 기준의 판정을 보여주고 어느 봉인지("HH:MM 봉 기준") 함께 적어요 — 이때는 매수하지 않아요.
