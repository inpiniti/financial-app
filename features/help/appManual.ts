...
- **대화(이 화면)** — 설명서만 읽는 게 아니라 필요하면 앱 데이터를 직접 조회해요: 자동 트레이딩 상태·감시 목록, 계좌 보유 종목·미체결, 오늘 매매 기록, 분봉과 추세 4선 판정, 일·주·월봉, 이벤트 로그(왜 안 샀는지), 지금 걸린 설정, 종목 시세·검색, 종목 기사 본문(Yahoo), 뉴스 검색, 웹 검색, 계좌 연결 진단, 그리고 원하면 **KIS API 원본 응답 JSON**까지 보여 줘요. 전부 **조회만** 하고, 주문·시작·정지·설정 변경은 하지 못해요. 그런 건 화면에서 직접 해야 해요.
