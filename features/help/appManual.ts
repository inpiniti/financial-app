...
- **홈 하단 메뉴 4칸**
  - **트레이딩** — 자동 매매의 본 화면. 상태 배지, 오늘 성과, 시작·정지 버튼, 그리드 관리 게이지, 트레이딩 리스트(종목마다 모델 판정 한 줄과 "매수 후보" 배지), 오늘 거래 기록, 기록(이벤트 로그). 모델 판정 줄은 확률과 상태를 함께 말해요 — "(참고)"는 정규장 밖 봉 판정(매수 없음), "거래대금 미달"·"봉 부족"은 그 종목이 걸러진 이유예요. 기준값과 "정규장에서만 매수"는 리스트 맨 위 설명 한 줄에 적혀 있어요. 확률이 떠 있는데 안 사는 건 기준값(과거 상위 1%)에 못 미쳐서예요. 장이 닫혀 새 봉이 없을 때는 마지막으로 받은 봉 기준의 판정에 "HH:MM 봉 기준"을 붙여요.
...
- **차트** — 종목 상세 차트의 분봉 기본값은 ${MODEL_BAR_MINUTES}분(모델이 쓰는 주기)이에요. 아직 안 끝난 봉은 **속을 비운 캔들**로 그리고 그 왼쪽에 점선을 그어 "여기까지가 확정된 봉"을 표시해요. 차트 아래에 **모델이 지금 이 종목을 어떻게 보는지**(확률·기준값·안 사는 이유)를 그대로 적어요 — 자동매매가 쓰는 것과 같은 계산이에요. 겹쳐 그린 이동평균 4선은 참고용이고 매매 판단에는 쓰지 않아요.
...
4. **진입** — 모델 확률이 기준값을 넘고 **그 종목이 매수 후보 안에** 있으면 사요. 신호가 났는데 사지 않았다면 이유가 **반드시 기록에 남아요** — 후보 밖(조용한 종목은 호가가 얇아 불리해요), 호가가 신호가보다 1% 넘게 도망감, 속도 하락, 현금 부족·쿨다운, 자리 만석, 미체결 연속 포기 뒤 쉬는 중 같은 사유가 그대로 적혀요. 기록에 아무것도 없다면 신호 자체가 없었던 거예요.
