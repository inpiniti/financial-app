...
//
// 판정 기준 (2026-08-12 use 컬럼 도입):
//   use = true  → 진입 가능 (승인은 개발자가 DB에서 직접 켠다)
//   use ≠ true  → 'pending' (등록은 됐지만 승인 대기)
//   is_active ≠ true → 'rejected' (차단된 계좌)
 ...
 * - is_active가 true가 아니면 'rejected'
 * - use가 true가 아니면 'pending' (승인 대기)
...
/**
 * 미등록 계좌의 등록 신청 — use=false로 넣는다(승인은 개발자가 DB에서 직접 켠다).
...
export async function registerAccount(
  ...
  try {
    const { error } = await client
      .from('approved_users')
      .insert({ account_no: accountNo, memo, use: false });
